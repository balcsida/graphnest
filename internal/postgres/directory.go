...
func (s *Store) APIPrincipal(ctx context.Context, tokenHash [32]byte, now time.Time) (authn.Principal, error) {
	...
	if err := tx.QueryRow(ctx, `update api_tokens token set last_used_at=$2
        ...
        returning token.user_id, token.repository_ids, token.delegation_only`, tokenHash[:], now).Scan(&userID, &ceiling, &delegationOnly); err != nil {
		return authn.Principal{}, err
	}
...
func (s *Store) CreateAPIToken(ctx context.Context, token authn.APITokenRecord) (int64, error) {
	var id int64
	err := s.pool.QueryRow(ctx, `insert into api_tokens (token_hash, prefix, user_id, repository_ids, delegation_only, created_at, expires_at) values ($1,$2,$3,$4,$5,$6,$7) returning id`, token.TokenHash[:], token.Prefix, token.UserID, token.RepositoryIDs, token.DelegationOnly, token.CreatedAt, token.ExpiresAt).Scan(&id)
	return id, err
}
...
func (s *Store) CreateAPITokenAudited(ctx context.Context, token authn.APITokenRecord, event audit.Event) (int64, error) {
	...
	if err := tx.QueryRow(ctx, `insert into api_tokens
		(token_hash, prefix, user_id, repository_ids, delegation_only, created_at, expires_at)
		values ($1,$2,$3,$4,$5,$6,$7) returning id`,
		token.TokenHash[:], token.Prefix, token.UserID, token.RepositoryIDs, token.DelegationOnly,
...
func (s *Store) ListAPITokens(ctx context.Context, userID int64) ([]authn.APITokenMetadata, error) {
	rows, err := s.pool.Query(ctx, `select id, prefix, repository_ids, delegation_only, created_at, last_used_at, expires_at from api_tokens where user_id=$1 and revoked_at is null order by id`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Prefix, &item.RepositoryIDs, &item.DelegationOnly, &item.CreatedAt, &item.LastUsedAt, &item.ExpiresAt); err != nil {
			return nil, err
		}
