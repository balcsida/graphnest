...
func TestRefreshRejectsInvalidGitHubCredentialWithoutIssuingTokens(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "GitHub rejects credential", ciphertext: true},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if response.Code != http.StatusBadRequest || body["error"] != "invalid_grant" || body["access_token"] != nil || body["refresh_token"] != nil {
				t.Fatalf("status=%d body=%v", response.Code, body)
			}
			if grant.RevokedAt == nil || grant.GitHubTokenCiphertext != nil {
				t.Fatalf("invalid GitHub credential did not revoke grant: before=%+v after=%+v", before, *grant)
			}
...
type failingRefreshRevocationStore struct {
	authn.OAuthStore
	err error
}

func (store *failingRefreshRevocationStore) RotateOAuthGrant(ctx context.Context, hash [32]byte, rotation authn.OAuthRotation) (authn.OAuthGrant, error) {
	if rotation.Revoke && store.err != nil {
		return authn.OAuthGrant{}, store.err
	}
	return store.OAuthStore.RotateOAuthGrant(ctx, hash, rotation)
}

func TestRefreshRevocationFailureIsRetryable(t *testing.T) {
	harness := newHarness(t)
	harness.github.err = unauthorizedError{}
	refresh, refreshHash, err := newSecret(nil, RefreshTokenPrefix)
	if err != nil {
	...
	before := *grant
	before.GitHubTokenCiphertext = append([]byte(nil), grant.GitHubTokenCiphertext...)
	store := &failingRefreshRevocationStore{OAuthStore: harness.store, err: errors.New("database unavailable")}
	harness.server.Store = store
	form := url.Values{"grant_type": {"refresh_token"}, "refresh_token": {refresh}, "client_id": {grant.ClientID}}

	response, body := harness.exchange(t, form)
	if response.Code != http.StatusServiceUnavailable || body["error"] != "temporarily_unavailable" || body["access_token"] != nil || body["refresh_token"] != nil ||
		grant.RevokedAt != nil || grant.AccessHash != before.AccessHash || grant.RefreshHash != before.RefreshHash ||
		grant.PreviousRefreshHash != before.PreviousRefreshHash || !bytes.Equal(grant.GitHubTokenCiphertext, before.GitHubTokenCiphertext) {
		t.Fatalf("failed revocation status=%d body=%v grant=%+v", response.Code, body, *grant)
	}
	store.err = nil
	response, body = harness.exchange(t, form)
	if response.Code != http.StatusBadRequest || body["error"] != "invalid_grant" || grant.RevokedAt == nil {
		t.Fatalf("revocation retry status=%d body=%v grant=%+v", response.Code, body, *grant)
	}
	if got := harness.store.github[grant.UserID]; !slices.Equal(got, []int64{101}) {
		t.Fatalf("shared GitHub grants changed: %v", got)
	}
