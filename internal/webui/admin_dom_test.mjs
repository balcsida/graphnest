...
const responses = {
  ...
  "/v1/admin/repositories": {repositories:[{github_id:7,name:"acme/repo",default_branch:"main",status:"mystery"}],truncated:true},
...
const requests = [];
let denyMutation = false;
globalThis.fetch = async (path, options = {}) => {
  requests.push({path, options});
  if (path === "/healthz" || path === "/readyz") return {ok:true,status:200};
  if (denyMutation && options.method === "POST") return {ok:false,status:403,json:async()=>({})};
...
const retry = all.find(node => node.textContent === "Retry");
...
assert.equal(storage.has("grepnest_admin_token"), false);
