...
func (*providerStore) Session(context.Context, [32]byte, time.Time) (authn.SessionRecord, error) {
	return authn.SessionRecord{}, errors.New("unused")
}
func (*providerStore) DeleteSession(context.Context, [32]byte) error { return nil }
...
func TestOIDCProviderCallbackSuccessConsumesCreatesSessionAndRedirects(t *testing.T) {
	...
	if !fixture.store.consumed || fixture.client.code != "good" || fixture.client.verifier != fixture.store.flow.CodeVerifier ||
		...
		fixture.store.session.DisplayName != "Ada" || fixture.store.session.Principal.Administrator {
...
func TestOIDCProviderCallbackConsumesBeforeTerminalFailures(t *testing.T) {
	tests := []struct {
		...
		{"denied group", "&code=good", func(f *callbackFixture) { f.provider.Mapper.AllowedGroups = []string{"admins"} }},
...
func newCallbackFixture(t *testing.T) *callbackFixture {
	...
	client := &providerClient{identity: authn.Identity{
		Provider: "oidc", Issuer: "https://issuer.example.test", Subject: "ada", DisplayName: "Ada", Groups: []string{"engineers"},
	}}
	provider := &Provider{
		Client: client, Store: store, Mapper: authn.ScopeMapper{AllowedGroups: []string{"engineers"}},
		Sessions: &authn.SessionManager{Store: store, TTL: time.Hour, Now: func() time.Time { return now }, Rand: bytes.NewReader(bytes.Repeat([]byte{4}, 32))},
