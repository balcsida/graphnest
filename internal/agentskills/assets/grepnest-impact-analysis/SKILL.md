...
3. Use `offset` for another page only when `partial` or the result cap requires it. Increase depth only when the review scope requires it.
...
| Status | Action |
|---|---|
| `ambiguous` | Record candidates; resolve before impact |
| `partial` | Report it; paginate within the agreed scope |
