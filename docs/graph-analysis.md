...
## Entity impact and public graph projections

The v2 analysis boundary exposes seven generation-scoped operations through `graphquery.Service`: `CallGraph`, `Usages`, `ImpactRadius`, `ExportedSymbols`, `FindByQualifiedName`, `ModuleStructure`, and `FilteredSubgraph`. `graphservice.Service` applies repository authorization and rechecks the selected commit before returning these values. HTTP, MCP, and CLI adapters remain separate transport work.

Call graphs traverse incoming and outgoing `calls`, `references`, `imports`, `instantiates`, and `navigates` relationships with independent direction-local discovery. Each direction retains the first original edge occurrence that discovers a node, and the merged graph can therefore retain both directions between the focal node and the same neighbor. Usages returns every incoming relationship, including `contains`, with one result per original edge occurrence.

Impact includes the focal entity, follows every incoming relationship except `contains`, and expands members of class, interface, struct, union, trait, protocol, module, and enum containers without spending a dependency hop. It keeps convergent and parallel evidence. The blast summary counts the retained bounded subgraph: distinct direct sources, non-focal entities within the requested hops, files, test files, route entities, and up to 40 files ordered by symbol count then path. When depth, fanout, node, or edge limits stop traversal, the response keeps these known counts and sets `partial` with explicit boundaries; callers must not interpret them as complete totals.

Exported-symbol results contain only facts with known `is_exported=true` and preserve source start-line order. Qualified-name lookup supports anchored `*` and `?` globs over the pinned public kinds. Module structure groups tracked file entities by directory, using `.` for repository-root files. Filtered subgraphs use bounded path/kind/export predicates and include an edge only when both endpoints are in the returned entity set.

PostgreSQL applies repository, upload, commit, and schema constraints to both entity filtering and endpoint-induced evidence queries. Each result preserves the original v2 protobuf fact and producer-derived identity. Qualified-name lookup uses the versioned discovery projection's original byte representation: literal queries use hash-plus-byte equality, while wildcard queries use byte-exact literal fragments and anchored prefixes or suffixes to select at most 1,002 candidates before the original protobuf fact is matched in Go. More than 1,001 candidates fail with `ErrQuerySize`; candidates are never silently truncated before authoritative case-sensitive matching. Repositories with an older discovery projection require its existing explicit rebuild path. The store also caps selector counts, regular-expression size, individual payload size, aggregate response size, and query duration; cancellation and generation drift abort without returning a mixed answer.

Every A3 response reports analysis state. `freshness=current` means current relative to the selected indexed commit. `complete=true` means the bounded query finished; `partial` and its boundaries independently report omitted graph results. `coverage=not_assessed` states that these operations do not establish producer or source-analysis coverage, and `unresolved` exposes the selected generation's unresolved-reference count.

Impact processes dependency levels and closes same-depth containment before advancing. This intentionally differs from the pinned source's depth-first traversal, which finalizes a node's first depth and can omit descendants when a later containment path reaches that node more cheaply. Accepted producer captures remain unchanged; GraphNest reports the shortest retained dependency depth and truthful within-hop reachability.

## Type relations and hierarchy

`graphquery.Service` adds two generation-scoped operations, `TypeRelations` and `TypeHierarchy`; `graphservice.Service` authorizes them and rechecks the selected commit like the other analysis operations. They are service-level only; REST, MCP, and browser exposure remain transport work.

Type relations return recorded outgoing `type_of`/`returns` targets, incoming `type_of` users, incoming `returns` returners, outgoing `references` whose target is a type kind, and recorded `overrides`. Each group keeps every original edge occurrence rather than collapsing repeated evidence. `type_knowledge=unknown` means the producer recorded no outgoing type facts for the entity; it is not a claim that the entity is untyped. The pinned portable producer emitted no `type_of` or `returns` edges for the accepted fixtures, so they report `unknown`.

Type hierarchy walks `extends` and `implements` for class, interface, struct, trait, protocol, enum, type alias, and union entities. Ancestors are complete to depth 8; descendants are breadth-first to depth 6 and at most 400 materialized rows, of which the viewer shows 24 ancestors and 240 descendants while reporting `total`, `shown`, and `truncated`. Each level lists `extends` before `implements`, then name, path, and line, as the pinned source does. A type tied to one supertype by both relations appears once, as `extends`. Cycles and duplicate occurrences are visited once per walk. When rows or depth stop the walk, `bounded`, `partial`, the boundary, and each parent's `hidden_subtypes` say so instead of presenting the last row as a leaf. Synthesized Go implicit implementations keep `synthesized`, `via`, `registered_at`, and their heuristic provenance.

`direct_subtypes` and `direct_implementers` count distinct direct subtypes in PostgreSQL, independent of the row bound, so they stay true when the list is truncated. A subtype with both relations counts as `extends`; the pinned source counts it by whichever edge it reads first. `polymorphic` is set at eight direct implementers.

Derived overrides match the focus's method, function, property, and field members by name against the nearest of up to 12 ancestors' members. They are always `signature_uncertain`, because overloads and signatures are not compared, and are reported separately from recorded `overrides` edges. Names sort by byte order; the pinned source uses locale comparison, which can order mixed-case names differently.

Permanent conformance uses the accepted real-producer capture in `test/fixtures/codegraph/type-hierarchy-facts.json` (37 nodes, 53 edges, 3 files, 5 unresolved references) with its four source files, whose hashes the integration test verifies, plus the pinned library fixture's `Base`/`Service` hierarchy. The capture was produced by the pinned CodeGraph at `b9ca4b7981116909900368cc1686a1074cd4d4c1` by indexing those sources twice and comparing both passes. Synthetic topologies cover bounds, cycles, direction, and recorded type relations; they are not producer output.
