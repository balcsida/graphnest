...
func run(ctx context.Context, settings config.Graph, logger *slog.Logger) error {
	...
	command := standaloneRuntime{
		...
		newGraph: func(ctx context.Context) error {
			var err error
			runtime, err = graphruntime.New(ctx, graphRuntimeConfig(settings), postgres.New(pool), logger)
			return err
		},
...
func graphRuntimeConfig(settings config.Graph) graphruntime.Config {
